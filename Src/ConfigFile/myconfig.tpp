...
template <typename T> void missingpoint(T parse) {
...
template <typename T> void Listen(deque_ &conf, T &fill) {
  ...
  if (pos == std::string::npos) {
    ...
    fill.listen.second =
        conf[0].substr(pos + 1, std::string(&conf[0][pos + 1]).length() - 1);
    if (ft_stoi(fill.listen.second) > 65535 ||
        ft_stoi(fill.listen.second) < 1024)
      throw std::runtime_error("Error: Port");
  }
  conf.pop_front();
}

template <typename T> void Root(deque_ &conf, T &fill) {
  if (conf[0][conf[0].length() - 1] == ';')
    missingpoint(conf[0]);
  else
    throw std::runtime_error("Error: Missing ; at the end");
...
template <typename T> void Allow(deque_ &conf, T &fill) {
  int count;
  for (count = 0; conf[0][conf[0].length() - 1] != ';' && count < 3; count++) {
    ...
    if (conf[0] != "GET" && conf[0] != "POST" && conf[0] != "DELETE")
      throw std::runtime_error("Error: Unknown HTTP method " +
                               conf[0].substr(0, conf[0].find(";")));
    conf.pop_front();
  }
  if (conf[0][conf[0].length() - 1] == ';')
    missingpoint(conf[0]);
...
template <typename T> void errorPage(deque_ &conf, T &fill) {
...
template <typename T> void maxbodySize(deque_ &conf, T &fill) {
  if (conf[0][conf[0].length() - 1] == ';')
    missingpoint(conf[0]);
  else
    throw std::runtime_error("Error: Missing ; at the end");
  ...
  if (fill.body_size.second != 'G' && fill.body_size.second != 'B' &&
      fill.body_size.second != 'M')
    throw std::runtime_error("Error: Invalid Char");
  conf.pop_front();
}

template <typename T> void uploadPath(deque_ &conf, T &fill) {
  if (conf[0][conf[0].length() - 1] == ';')
    missingpoint(conf[0]);
  else
    throw std::runtime_error("Error: Missing ; at the end");
...
template <typename T> void autoindex(deque_ &conf, T &fill) {
  if (conf[0][conf[0].length() - 1] == ';')
    missingpoint(conf[0]);
  else
    throw std::runtime_error("Error: Missing ; at the end");
  conf[0] = conf[0].substr(0, conf[0].length() - 1);
  if (conf[0] != "ON" && conf[0] != "OFF" && conf[0] != "on" &&
      conf[0] != "off")
...
template <typename T> void serverName(deque_ &conf, T &fill) {
  if (conf[0][conf[0].length() - 1] == ';')
    missingpoint(conf[0]);
  else
    throw std::runtime_error("Error: Missing ; at the end");
...
template <typename T> void cgi(deque_ &conf, T &fill) {
...
template <typename T> void indexfiles(deque_ &conf, T &fill) {
...
template <typename T> void redirect(deque_ &conf, T &fill) {
