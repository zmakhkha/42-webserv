...
void request::reExecCgi() {
  obj.excecCgi(cgiBodyPath);
  if (obj.cgiDone) {
...
void request::handleCgi(const st_ &data) {
  ...
  if (cgi) {
    ...
    // obj = tmp;
    obj.setUri(getURI());
    obj.setMethode(getMethod_());
    obj.setLocation(locate);
    obj.setRespPath(cgiResult);
    obj.setReqHeaders(getVector());
    obj.setUploadPath(upPath);
    obj.setServer(Serv);
    std::cout << obj.getUri() << std::endl;
...
void request::feedMe(const st_ &data) {
  try {
    ...
    if (!Serv.location[locate].allow.Post && getMethod_() == "POST")
      throw 405;
    upPath = Serv.location[locate].up_path;
    if ((getURI().rfind(".py") != std::string::npos ||
         getURI().rfind(".php") != std::string::npos) &&
    ...
    if (cgi) {
      ...
      if (getMethod_() == "GET" || getMethod_() == "DELETE")
        ...
        readBits += data.size();
        upDone = readBits >= atol(headers["content-length"].c_str());
        if (upDone && reading == 0) {
          throw 201;
        }
      }
